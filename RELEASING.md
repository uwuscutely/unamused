...
2. Make sure your GitHub Actions repository secrets include `DOCKERHUB_TOKEN` and `DOCKERHUB_PASSWORD` for publishing to `victorruwu/unamused` on Docker Hub. Releases are also published to `ghcr.io/uwuscutely/unamused` using GitHub's automatic `GITHUB_TOKEN`.
